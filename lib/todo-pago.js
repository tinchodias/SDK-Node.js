...
module.exports = {
	...
	sendAutorizeRequest : function (options, parameters, fraudControl, callback){
		...
		var config = {
			"endpoint": options.endpoint + soapAppend + tenant + soapAthorizeAppend
		};
	...
	getStatus : function (options, merchant, operationId, callback){
		var Client = require('node-rest-client').Client;
 		client = new Client();
 		client.get(options.endpoint + tenant + restAppend + "Operations/GetByOperationId/MERCHANT/" + merchant + "/OPERATIONID/" + operationId, function(data, response){
	...
	getPaymentMethods : function (options, merchant, callback){
		var Client = require('node-rest-client').Client;
 		client = new Client();
		client.get(options.endpoint + tenant + restAppend + "/PaymentMethods/Get/MERCHANT/" + merchant, function(data, response){
			var aux = data.toString('utf-8');
			var parseString = require('xml2js').parseString;
			parseString(aux, function (err, result) {
				var ret = {
